...
Konfigurasi staging menerima payload product PIM pada `POST https://eiger-dev.automataserver.com/api/integrations/pim/product` dan payload image pada `POST https://eiger-dev.automataserver.com/api/integrations/pim/image`. Keduanya menerima bentuk payload asli dokumentasi PIM dan memakai Bearer token yang sama. Product dikirim lebih dahulu, lalu image secara sinkron. Setelah payload product lolos validasi, CMS langsung meminta harga ke CARE Master dan stok per bin ke CARE WMS, lalu menyimpan data PIM dan CARE sebagai satu katalog artikel. Shared folder di bagian berikut tetap tersedia sebagai pilihan alternatif.
...
- Penerima memerlukan Bearer token sementara. Buat token dua jam dengan `php artisan pim:issue-token --ttl=120 --name=EIGER-PIM`, atau tentukan waktu dengan `--expires-at="2026-09-19 00:00:00"` (maksimal tujuh hari). Nilai token hanya ditampilkan sekali dan database hanya menyimpan hash SHA-256. Token aktif lama dengan nama yang sama otomatis dicabut. Setelah `expires_at`, request menerima HTTP 401. Token statis lama hanya diterima jika `PIM_ALLOW_STATIC_INBOUND_TOKEN=true` disetel secara eksplisit.
- CARE menggunakan dua host sesuai dokumen September 2026: `CARE_MASTER_URL` untuk `/api/server/pricing_details` dan `CARE_WMS_URL` untuk `/api/server/inventories/bybin`. Keduanya memakai header `x-server-key` dan kode toko `CARE_STORE_CODE`. Pengayaan PIM dan sinkronisasi massal hanya memakai endpoint CARE resmi ini; tidak ada fallback ke simulator.
- Pencarian harga CARE Master memakai query `page[size]`, `page[number]`, `filter[skucode]`, dan opsional `filter[loccode]`. Pencarian stok CARE WMS wajib memakai `filter[location]` serta `filter[search_sku.skucode]` dengan SKU varian 12 digit. Parameter `skucode` biasa tanpa `filter[...]` diabaikan oleh staging dan dapat mengembalikan seluruh katalog. CMS juga menyaring respons sekali lagi berdasarkan kecocokan SKU persis agar baris lain tidak pernah diterapkan ke produk.
...
- Endpoint product menerima payload Product Detail secara langsung. Endpoint image menerima objek `generic[]` dan `variant[]` secara langsung; format lama `{ "product": {...}, "image": {...} }` tetap didukung pada endpoint product untuk kompatibilitas.
